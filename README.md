# ChatbotWebsite

> **Built in December 2023** -- an early AI chatbot website project, created when the builder was 17 years old and already deep into AI development, before AI-assisted coding went mainstream.

## About

This is a React + Vite scaffold for an AI chatbot web interface, created in Visual Studio 2022. It represents the starting point of a chatbot website project from late 2023 -- a time when building AI-powered applications was still a niche pursuit, especially for a teenager.

The project captures the initial scaffolding phase: a Vite + React frontend configured inside a Visual Studio solution, ready to be wired up to a chatbot backend.

## Tech Stack

- **React** 18.2 -- UI framework
- **Vite** 5.0 -- Build tool and dev server
- **ESLint** -- Code linting with React plugin
- **Visual Studio 2022** -- Solution wrapper (`.sln` + `.esproj`)

## Timeline

| Date | Event |
|------|-------|
| December 18, 2023 | Project created |
| March 2026 | Archived and published to GitHub |

## Archive Notice

This repository is archived for historical purposes. It is not actively maintained.

## License

MIT
